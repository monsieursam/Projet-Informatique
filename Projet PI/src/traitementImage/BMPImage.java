...
public class BMPImage {
	//Constantes
	/**Constante de masque pour la comparaison bit à bit*/
	...
	@param in Flux d'entrée
	@return Entier lu*/
	public static int readColor(DataInputStream in){
		byte[] b = new byte[3];
		int result = 0;
		try{
			in.read(b);
	...
	public static boolean writeBMP(BufferedImage img, String filename) {
		try{
			...
			for (int y = resy - 1; y >= 0; y--) {
				...
				//Bourrage
				for (int j = 0; j < sup; j++) outBMP.writeByte(0);
			}
