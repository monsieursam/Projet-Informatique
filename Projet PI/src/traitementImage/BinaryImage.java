...
public class BinaryImage {
	...
	/**
	 * Constructor that instantiates a square image of a specified size (all
	 * pixels are black)
	 */
	...
	public BinaryImage(String filename) {
		...
		if (width == -1) {
			...
			throw new IllegalArgumentException("Error in opening the file "
					+ filename);
		}
	}
	...
	public java.awt.Image toImage() {
		ImageProducer ip = new MemoryImageSource(width, height, raster, 0,
				width);
	...
	/**
	 * Fill the pixel of a squared area with black color. The area is defined by
	 * its top left corner P=(x,y) and by its size.
	 */
	...
	/**
	 * Fill the pixel of a squared area with white color. The area is defined by
	 * its top left corner P=(x,y) and by its size.
	 */
